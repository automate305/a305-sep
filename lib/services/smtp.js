...
export function senderIsEligible(sender, environment = process.env) {
  const passwordEnvironmentVariable = getSmtpPasswordEnvironmentVariable(sender.email)

  return Boolean(sender.active) &&
    Boolean(sender.warmed) &&
    senderHasCapacity(sender) &&
    isUsableEnvironmentValue(environment[passwordEnvironmentVariable])
}
